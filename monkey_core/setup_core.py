...
from core.setup_scripts.utils import Completer, get_monkey_fs
...
def add_provider_cloud_args(parser):
    ...
    parser.add_argument('--filesystem-only',
                        action='store_true',
                        required=False,
...
def main():
    ...
    print("{} providers found: {}".format(
        len(providers), ", ".join([x.get("name", "unknown") for x in providers])))
    args = parse_args()
    ...
    if create:
        ...
        if not args.noinput and not args.provider_name:
            provider_name = input(
                "Provider name? ({}) : ".format(provider_name)) or provider_name
