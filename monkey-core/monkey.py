...
class Monkey():
        ...
        for job in pending_jobs:
            ...
            if job.state == MONKEY_STATE_DISPATCHING_MACHINE:
                ...
                if time_elapsed > MONKEY_TIMEOUT_DISPATCHING_MACHINE:
                    print("Found DISPATCHING_MACHINE with time: ",
                          time_elapsed, "\n\nRESETTING TO QUEUED\n\n")
                ...
                if time_elapsed > MONKEY_TIMEOUT_DISPATCHING_INSTALLS:
                    print("Found DISPATCHING_INSTALLS with time: ",
                          time_elapsed, "\n\nRESETTING TO QUEUED\n\n")
                    job.set_state(state=MONKEY_STATE_QUEUED)
                ...
                if (job.run_timeout_time != -1 and job.run_timeout_time != 0) \
                    ...
                    logger.info(
                        "Reached maximum running time: {}.  Killing job".
                        format(job.job_uid))
    ...
    def run_job(self, provider, job_yml):
        ...
        if success == False:
            ...
            return success, msg
        dbMonkeyJob.run_elapsed_time += (
            datetime.now() -
            dbMonkeyJob.run_running_start_date).total_seconds()
    ...
    def get_image_list(self):
        ...
        return {
            handler.name: handler.list_images()
            for handler in self.providers
        }
